...
void CGame::runGame()
{
    ...
    while(doGameTurn(field, output, player1, player2)){
        ++this->roundNumber;
		output->printRndNmbr(roundNumber);
	}
...
bool CGame::doGameTurn(CField* field_, COutput* output_, CPlayer* player1_, CPlayer* player2_)
{
	if(!player1_->move(calcRand(0, 100), field_)){
		output_->printCollision(player1_, player2_);
		output_->printWin(player2_);
		return false;
	}
    if(!player2_->move(calcRand(0, 100), field_)){
		output_->printCollision(player2_, player1_);
		output_->printWin(player1_);
		return false;
	}
